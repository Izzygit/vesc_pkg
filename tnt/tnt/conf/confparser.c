...
int32_t confparser_serialize_tnt_config(uint8_t *buffer, const tnt_config *conf) {
	...
	buffer[ind++] = (uint8_t)conf->wheelslip_filter_freq_fast;
	buffer[ind++] = (uint8_t)conf->wheelslip_filter_freq_slow;
	buffer[ind++] = (uint8_t)conf->wheelslip_max_angle;
	buffer[ind++] = conf->is_traction_braking_enabled;
	buffer_append_float16(buffer, conf->traction_braking_angle, 10, &ind);
...
bool confparser_deserialize_tnt_config(const uint8_t *buffer, tnt_config *conf) {
	...
	conf->wheelslip_filter_freq_fast = buffer[ind++];
	conf->wheelslip_filter_freq_slow = buffer[ind++];
	conf->wheelslip_max_angle = buffer[ind++];
	conf->is_traction_braking_enabled = buffer[ind++];
	conf->traction_braking_angle = buffer_get_float16(buffer, 10, &ind);
...
void confparser_set_defaults_tnt_config(tnt_config *conf) {
	...
	conf->wheelslip_filter_freq_fast = APPCONF_TNT_WHEELSLIP_FILTER_FREQ_FAST;
	conf->wheelslip_filter_freq_slow = APPCONF_TNT_WHEELSLIP_FILTER_FREQ_SLOW;
	conf->wheelslip_max_angle = APPCONF_TNT_WHEELSLIP_MAX_ANGLE;
	conf->is_traction_braking_enabled = APPCONF_TNT_IS_TRACTION_BRAKING_ENABLED;
	conf->traction_braking_angle = APPCONF_TNT_TRACTION_BRAKING_ANGLE;
