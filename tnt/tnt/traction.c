	...
	if (state->wheelslip) {
		if (rt->current_time - traction->timeron > .8) {		// Time out at 500ms
			...
			if (traction->highaccelon2) {
				if (sign(traction->accelstartval) * m->accel_fast < traction->slowed_accel) {	 	
			...
			} else if (fabsf(m->accel_fast) > traction->end_accel) {
